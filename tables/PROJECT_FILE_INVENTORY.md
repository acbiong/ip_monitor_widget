...
| src/operator_display.py | operator_display.py | 文件 | 运营商中文显示和悬停详情格式，双栈分行及断网降级。 |
| src/operator_lookup.py | operator_lookup.py | 文件 | 公网地址归属查询工作进程：验证前缀/ASN、中文运营商映射及协议限长。 |
| src/operator_service.py | operator_service.py | 文件 | 运营商异步队列、进程超时、地址去重缓存、旧结果隔离及退出清理。 |
